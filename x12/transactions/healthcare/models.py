...
class Claim837P(BaseModel):
    ...
    def to_edi(self, delimiters: "Delimiters") -> str:
        ...
        parts.append(f"BHT*0019*00*{self.claims[0].claim_id if self.claims else 'REF'}*20231127*1200*CH{d.segment}")
        
        # 2000A - Billing Provider
        parts.append(f"HL*1**20*1{d.segment}")
        parts.append(f"NM1*85*2*{self.billing_provider.name}*****XX*{self.billing_provider.npi}{d.segment}")
        
        # 2000B - Subscriber
        parts.append(f"HL*2*1*22*0{d.segment}")
        parts.append(f"NM1*IL*1*{self.subscriber.last_name}*{self.subscriber.first_name}****MI*{self.subscriber.member_id}{d.segment}")
